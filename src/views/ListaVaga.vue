...
<template>
  <div class="bg-white relative flex w-screen  overflow-x-hidden overflow-y-scroll h-screen">
    ...
    <div class="w-[90%] border-b-[2px] flex flex-row border-black ">
      <input
      v-model="namesearch" 
      type="text" class="w-[90%] pl-5 outline-none hover:hover:bg-[#f7f6f6] rounded-xl">
      <div
      id="search"
      v-if="!isSeach"
      @click="SearchName"
      ...
      <LabelLista name="Vagas"  estilo="bg-[#FFD600] w-[7rem] absolute font-bold top-[8.7rem] left-[9rem] shadow-md rounded-lg text-center z-10"/>
      <LabelLista name="Nível" estilo="bg-[#FFD600] w-[7rem] absolute font-bold top-[8.7rem] right-[32rem] shadow-md rounded-lg text-center z-10"/>
      <LabelLista name="Visualizar" estilo="bg-[#FFD600] w-[7rem] absolute font-bold top-[8.7rem] right-[7rem] shadow-md rounded-lg text-center z-10"/>
      <section
      v-if="!isSeach"
      ...
      >
      <div
      class="items-center flex justify-center mt-[20vh]"
      v-if="loading">
        <LoaderforList/>
      </div>
        <li 
        v-else
        v-for="vaga in vagas" :key="vaga.id"
